...
//        Tenant tenant = tenantClient.getTenantByUsername(token.getPrincipal());
//        if (Objects.isNull(tenant)) {
//            throw new ServiceException(DefaultResultType.NOT_EXIST);
//        }
//        // 匹配密码
//        if (!passwordEncoder.matches(token.getCredentials(), tenant.getPassword())) {
//            throw new ServiceException(DefaultResultType.USERNAME_PASSWORD_INCORRECT);
//        }
//        // 查询权限
//        List<TenantPermission> list = tenantClient.getPermissionList(tenant.getId());
...
//        }
//        // 返回 token
//        return new TenantAuthenticationToken(permission, tenant.getUsername(), tenant.getPassword(), tenant.getId());
//    }
//
//    @Override
